...
public class QuizDetailResponse {
    ...
    public static QuizDetailResponse of(QuizAttempt attempt, List<QuizQuestion> questions, List<QuizChoice> allChoices) {
        return QuizDetailResponse.builder()
                .attemptId(attempt.getId())
                .quizSetId(attempt.getQuizSet().getId())
                .status(attempt.getStatus().name())
                .questions(questions.stream().map(q -> {
                    List<ChoiceDetail> choices = allChoices.stream()
                            .filter(c -> c.getQuizQuestion().getId().equals(q.getId()))
                            .map(c -> ChoiceDetail.builder()
                                    .choiceId(c.getId())
                    ...
                    return QuestionDetail.builder()
                            ...
                            .choices(q.getQuestionType() == QuestionType.MULTIPLE_CHOICE ? choices : null)
                            .build();
                }).collect(Collectors.toList()))
                .build();
    }
