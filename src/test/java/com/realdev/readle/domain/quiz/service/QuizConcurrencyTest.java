...
class QuizConcurrencyTest {
    ...
    @MockBean
    private com.realdev.readle.global.infrastructure.ai.ClaudeClient claudeClient;
    ...
    void setUp() {
        ...
        jdbcTemplate.update("INSERT INTO member (uuid, oauth_provider, oauth_id, nickname, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())",
                "test-uuid-conc", "KAKAO", "test-conc", "conc");
        Long memberId = jdbcTemplate.queryForObject("SELECT MAX(id) FROM member", Long.class);

        jdbcTemplate.update("INSERT INTO content (member_id, title, input_type, crawl_status, raw_text, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())",
                memberId, "Test Title", "TEXT", "NOT_APPLICABLE", "Public void code() {}");
        Long contentId = jdbcTemplate.queryForObject("SELECT MAX(id) FROM content", Long.class);

        jdbcTemplate.update("INSERT INTO content_validation (content_id, validation_method, status, validation_score, created_at) VALUES (?, ?, ?, ?, NOW())",
                contentId, "AI", "PASSED", 90.0);
        Long validationId = jdbcTemplate.queryForObject("SELECT MAX(id) FROM content_validation", Long.class);
