...
EC2 보안 그룹에서 인터넷 인바운드로 필요한 포트는 다음과 같다.

- 22 (신뢰된 관리자 IP 등으로 제한)
- 80
- 443

`3000`, `8000-8100`은 Readle 서비스의 인터넷 인바운드 요구 사항이 아니다. legacy 보안 그룹 규칙이 남아 있으면 보안 그룹 권한 보유자가 제거하거나 신뢰된 내부 관리 source로 제한한다. 현재 배포에 사용하는 AWS 접근 권한은 보안 그룹 수정 권한을 갖고 있지 않다.

프론트엔드와 백엔드는 Nginx 뒤의 Podman 내부 서비스로만 운영한다. Grafana도 host port `3000`을 publish하지 않고 Nginx의 `/grafana/` subpath로만 제공하며, Prometheus·Loki·exporter는 host port를 publish하지 않는다.
...
프론트엔드 배포는 `readle-frontend-candidate`로 비라우팅 preflight를 수행한 뒤 `readle-frontend` 단일 live를 교체하고, 실패 시 durable previous state로 되돌린다. Loki/Alloy 포함 frontend-only 배포 중 memory hard cap 합계는 `3392MB`이고, 4GiB host에서 산술상 잔여 한도는 약 `704MB`다. 이는 cgroup 상한의 합계일 뿐 보장된 host reserve는 아니다.

Loki/Alloy 포함 backend candidate 배포 중 memory hard cap 합계 `3840MB`는 staged exception이다. 4GiB host에서 산술상 잔여 한도는 약 `256MB`지만, cgroup 상한은 예약량이 아니므로 이 값으로 실제 `MemAvailable`을 추론하지 않는다. 이 값은 평시 계약이 아니며, enable 전 실제 candidate와 logging stack을 띄운 뒤 3분 안정화 구간을 거쳐 15분 동안 `MemAvailable >= 576MB`, 분당 swap-in `<= 1MiB`, memory PSI `some`·`full`의 `avg10 < 0.10`, 모든 sample의 5-minute load average `< 1.5`, root filesystem 여유 `10GB` 이상 및 `20%` 이상, OOM 없음이 확인되어야 한다. 이 실제 capacity gate가 운영상 reserve를 판단한다. swap-out은 evidence만 기록하며 단독 실패 조건으로 사용하지 않는다. frontend/backend candidate 동시 배포는 지원하지 않는다.
